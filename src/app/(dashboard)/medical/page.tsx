                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredHealthRecords.map(r => (
                    <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{r.class}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {r.allergies.map(a => (
                            <span key={a} className={`px-2 py-0.5 rounded-full text-xs font-medium ${allergyColors[a] || "bg-slate-100 text-slate-600 dark:text-slate-400"}`}>{a}</span>
                          ))}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{r.conditions}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{r.lastCheckup}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredClinicVisits.map(v => (
                    <tr key={v.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{v.complaint}</td>
                      <td className="px-4 py-3 text-sm text-slate-700 font-medium">{v.diagnosis}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{v.treatment}</td>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredMedications.map(m => (
                    <tr key={m.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{m.dose}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{m.frequency}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{m.startDate}</td>
                      <td className="px-4 py-3">
                        <span className={`text-sm font-medium ${m.endDate === "Ongoing" ? "text-amber-600" : "text-slate-600 dark:text-slate-400"}`}>{m.endDate}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleVisitSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input value={visitForm.student} onChange={e => setVisitForm({ ...visitForm, student: e.target.value })} required className="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="Student name" />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" className="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 transition-all text-sm font-medium flex-1">Record Visit</button>
