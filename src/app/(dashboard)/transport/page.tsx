    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage school vehicles, routes, and drivers</p>
        </div>
        <button onClick={() => setShowVehicleModal(true)} className="flex items-center gap-2 bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 transition-colors text-sm font-medium">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredVehicles.map((v, i) => (
                  <tr key={i} className="hover:bg-slate-50">
                    ...
                    <td className="py-3 px-4 text-sm text-slate-600">{v.driver}</td>
                    <td className="py-3 px-4 text-sm text-slate-600">{v.route}</td>
            ...
            <div key={i} className="bg-white rounded-xl p-5 shadow-sm border border-slate-200">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1 flex items-center gap-1"><MapPin className="w-3 h-3" /> {r.stops}</p>
                </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredDrivers.map((d, i) => (
                  <tr key={i} className="hover:bg-slate-50">
                    ...
                    <td className="py-3 px-4 text-sm text-slate-600 font-mono">{d.license}</td>
                    <td className="py-3 px-4 text-sm text-slate-600">{d.phone}</td>
                    <td className="py-3 px-4 text-sm text-slate-600 font-mono">{d.vehicle}</td>
                    <td className="py-3 px-4">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h2 className="text-lg font-bold text-slate-800">Add Vehicle</h2>
              <button onClick={() => setShowVehicleModal(false)} className="p-2 hover:bg-slate-100 rounded-lg"><X className="w-5 h-5 text-slate-500" /></button>
            </div>
            ...
            <div className="flex gap-3 p-6 border-t border-slate-200">
              <button onClick={() => setShowVehicleModal(false)} className="flex-1 border border-slate-200 text-slate-700 py-2 rounded-lg hover:bg-slate-50 text-sm font-medium">Cancel</button>
