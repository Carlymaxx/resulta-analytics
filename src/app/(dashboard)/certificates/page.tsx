              ...
              <form onSubmit={handlePreview} className="space-y-6">
                <div>
                  ...
                  <div className="space-y-3">
                    {certTypes.map(ct => (
                      <label key={ct.id} className={`flex items-start gap-3 p-4 border rounded-xl cursor-pointer transition-colors ${selectedType === ct.id ? "border-teal-500 bg-teal-50" : "border-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50"}`}>
                ...
                <div>
                  ...
                  <select value={studentClass} onChange={e => setStudentClass(e.target.value)} className="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500">
                      {levelClasses.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <button type="submit" className="bg-teal-600 text-white px-6 py-2.5 rounded-lg hover:bg-teal-700 transition-all text-sm font-medium flex items-center gap-2">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredHistory.map(c => (
                    <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{c.class}</td>
                      <td className="px-4 py-3"><span className="px-2 py-1 bg-teal-50 text-teal-700 rounded-full text-xs">{c.type}</span></td>
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{c.date}</td>
                      <td className="px-4 py-3 text-sm text-slate-500 dark:text-slate-400">{c.issuedBy}</td>
                      <td className="px-4 py-3 flex gap-2">
                        <button onClick={() => alert("PDF export coming soon")} className="border border-slate-200 text-slate-700 px-2 py-1 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-xs font-medium flex items-center gap-1">
              ...
              {templates.map(t => (
                <div key={t.id} className="border border-slate-200 rounded-xl p-5 hover:shadow-sm transition-shadow">
                  ...
                  <div className="flex gap-2">
                    <button className="border border-slate-200 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-50 transition-all text-sm font-medium">Preview</button>
                    <button className="border border-slate-200 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-50 transition-all text-sm font-medium">Edit</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h2 className="text-lg font-semibold text-slate-800 dark:text-white">Certificate Preview</h2>
              ...
              <div className="border-4 border-teal-600 rounded-xl p-8 text-center relative">
                <div className="border-2 border-teal-200 rounded-lg p-6">
                  ...
                  <div className="text-3xl font-bold text-slate-800 border-b-2 border-teal-300 pb-2 mb-4 px-8">{studentName}</div>
                   <div className="text-sm text-slate-600 dark:text-slate-400 mb-1">of grade <span className="font-semibold">{studentClass}</span></div>
                  <div className="text-xl font-bold text-teal-600 my-4">{selectedTypeLabel}</div>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-6 max-w-md mx-auto">{certBodyText[selectedType]}</p>
            ...
            <div className="flex gap-3 px-8 pb-6">
              <button onClick={() => alert("PDF export coming soon")} className="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 transition-all text-sm font-medium flex items-center gap-2 flex-1 justify-center">
                <Printer className="w-4 h-4" /> Print Certificate
              </button>
              <button onClick={() => alert("PDF export coming soon")} className="border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-all text-sm font-medium flex items-center gap-2">
                <Download className="w-4 h-4" /> Download PDF
              </button>
              <button onClick={() => setShowPreview(false)} className="border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-all text-sm font-medium">Close</button>
            </div>
          </div>
        </div>
      )}
