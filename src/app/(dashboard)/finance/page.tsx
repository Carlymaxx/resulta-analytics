              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredFeeStructure.map(fee => (
                    <tr key={fee.category} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{fee.students}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredPayments.map(p => (
                    <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{p.class}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-slate-500 dark:text-slate-400 font-mono text-xs">{p.receipt}</td>
                      <td className="px-4 py-3">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredInvoices.map(inv => (
                    <tr key={inv.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      <td className="px-4 py-3 text-sm font-mono text-slate-600 dark:text-slate-400">{inv.id}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{inv.due}</td>
                      <td className="px-4 py-3">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredExpenses.map(exp => (
                    <tr key={exp.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      <td className="px-4 py-3"><span className="px-2 py-1 bg-slate-100 rounded-full text-xs text-slate-600 dark:text-slate-400">{exp.category}</span></td>
                      <td className="px-4 py-3 text-sm font-semibold text-slate-800">{exp.amount.toLocaleString()}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{exp.date}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{exp.approvedBy}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handlePaySubmit} className="p-6 space-y-4">
              <div>
                ...
                <input value={payForm.student} onChange={e => setPayForm({ ...payForm, student: e.target.value })} required className="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="Student name" />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" className="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 transition-all text-sm font-medium flex-1">Record Payment</button>
